...
namespace
{
        ...
        : padding(pad), forEncrypt(encrypt)
    {
        ...
        const auto ret = encrypt ? EVP_PKEY_encrypt_init(this->pctx) : EVP_PKEY_decrypt_init(this->pctx);
        if (ret <= 0) [[unlikely]]
        {
            dynxxLogPrintF(Error, "RSA {} EVP_PKEY context init failed, err: {}", encrypt ? "encrypt" : "decrypt", errMsg());
    ...
    Bytes RsaPkeyCodec::process(BytesView in) const
    {
        ...
        const auto ret = this->forEncrypt ? EVP_PKEY_encrypt(this->pctx, outBytes.data(), &outLen, in.data(), in.size())
                                          : EVP_PKEY_decrypt(this->pctx, outBytes.data(), &outLen, in.data(), in.size());
        if (ret <= 0) [[unlikely]]
        {
