...
def main():
    ...
    linux_abi = os.environ.get("LINUX_ABI", "x64")

    build_folder = f"build.{platform_name}/{build_type}"
    output_folder = f"{build_folder}/output"
    output_path = (root / output_folder / linux_abi).as_posix()
    ...
    os.environ["VCPKG_TARGET_TRIPLET"] = os.environ.get("VCPKG_TARGET_TRIPLET", f"{linux_abi}-linux")
    ...
    merge_libs(output_lib_path, "libDynXX-All.a", "ar")
